...
**Active open questions:** 2
**Total resolved:** 58 (OQ-001 through OQ-058)

---

## This Run
- Added: 2 new questions (OQ-059 to OQ-060)
...
### OQ-059 — Payment.bookingId constraint vs P1 package purchases

**Status:** Resolved
**Raised:** 2026-03-29
**Source:** critique-proposed.md (CR-003), data-model.md Payment entity
**Blocks:** UC-024 (P1 — Purchase a lesson package)

**Question:** The current Payment schema has `CHECK: bookingId IS NOT NULL`. UC-024 (P1) creates a Payment for a LessonPackage purchase — there is no bookingId at the time of package purchase. How should the Payment entity accommodate both booking payments and package purchase payments?

**Why it matters:** Without resolving this, the P1 package billing flow is impossible to implement without a schema migration that may require revisiting the payment model. The constraint must be relaxed or an alternative entity introduced before P1 development begins.

**Options:**
1. Make `Payment.bookingId` nullable; add `Payment.lessonPackageId` nullable FK; enforce `(bookingId IS NOT NULL) OR (lessonPackageId IS NOT NULL)` — **data-model-proposed.md adopts this approach provisionally**
2. Introduce a separate `PackagePayment` entity to avoid coupling package and booking payment flows
3. Keep Payment as-is and introduce a stub Booking record for package purchases (not recommended — semantic mismatch)

**Decision:** Introduce a separate `PackagePayment` entity to avoid coupling package and booking payment flows.  This will necessitate a flow to redeem each lesson from a package.  Which would essentially look like credits deducted in the checkout flow.

---

### OQ-060 — Booking auto-completion: what if instructor never marks lesson complete?

**Status:** Resolved
**Raised:** 2026-03-29
**Source:** critique-proposed.md (Significant Gaps), UC-013
**Blocks:** UC-007 (rating), booking.completed notification event, earnings reporting accuracy

**Question:** UC-013 requires an instructor to manually tap Complete to transition `Booking.status` from `confirmed` to `completed`. If the instructor never does this, the booking stays `confirmed` indefinitely. What is the fallback?

**Why it matters:** Students can never submit a rating (requires `completed` status). The `booking.completed` event never fires, so the post-lesson review email is never sent. Earnings reports undercount completed lessons. Ops has no visibility without an explicit state.

**Options:**
1. Auto-complete: a scheduled job transitions `confirmed` bookings to `completed` after `endAt + N hours`
2. Ops workflow: a background alert flags bookings past `endAt` that are still `confirmed`; admin manually resolves
3. Admin override only: add `PATCH /api/v1/bookings/:id/complete` to `school_admin` auth scope, not just `instructor`

**Decision:** 1. Auto-complete: a scheduled job transitions `confirmed` bookings to `completed` after `endAt + 2 hours`
---
